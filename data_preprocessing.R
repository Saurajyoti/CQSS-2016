...
list4 <- read_csv("../../Media/2016/CSV/Sheet_3.csv")
...
bigtable$StartDate_ <- as.POSIXct(bigtable$StartDate, format = "%m/%d/%Y %H:%M:%S")
bigtable$EndDate_ <- as.POSIXct(bigtable$EndDate, format = "%m/%d/%Y %H:%M:%S")

#######################################################################################################
write.csv(x = bigtable, file = paste0("../../Media/2016/Master_tables/", Sys.Date(), "-bigtable.csv"), fileEncoding = "UTF-8")
#check other please specify with university 4
